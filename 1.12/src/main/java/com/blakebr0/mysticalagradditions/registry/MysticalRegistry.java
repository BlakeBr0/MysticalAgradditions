package com.blakebr0.mysticalagradditions.registry;
...
import com.blakebr0.mysticalagriculture.registry.RegistryObject;
		...
		for(RegistryObject<Item> item : items){
			if(item.get() instanceof ItemMeta){
				...
				if(((ItemBlock)item.get()).getBlock() instanceof IModelHelper){
					...
					ModelLoader.setCustomModelResourceLocation(item.get(), 0, new ModelResourceLocation(MysticalAgradditions.MOD_ID + ":" + item.getName(), "inventory")); 
				}
			} else {
				ModelLoader.setCustomModelResourceLocation(item.get(), 0, new ModelResourceLocation(MysticalAgradditions.MOD_ID + ":" + item.getName(), "inventory")); 
			}
